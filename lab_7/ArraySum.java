...
public class ArraySum {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            // "Если это ПОСЛЕДНИЙ помощник (i равен numberOfThreads-1),
            // то пусть считает ДО САМОГО КОНЦА массива (array.length).
            // Иначе пусть считает ровно свой кусок (start + chunkSize)."
            System.out.printf("Поток %d обрабатывает элементы с %d по %d (всего %d элементов)%n",  i + 1, start, end - 1, end - start);
